defmodule JidoTest.AgentServer.OptionsTest do
  ...
  describe "new/1 with keyword list" do
    ...
    test "creates options with custom id" do
      {:ok, opts} = Options.new(agent: ValidAgent, id: "custom-id")

      assert opts.id == "custom-id"
    end

    test "creates options with initial_state" do
      {:ok, opts} = Options.new(agent: ValidAgent, initial_state: %{foo: :bar})

      assert opts.initial_state == %{foo: :bar}
    end

    test "creates options with custom registry" do
      {:ok, opts} = Options.new(agent: ValidAgent, registry: MyRegistry)

      assert opts.registry == MyRegistry
    end

    test "creates options with default_dispatch" do
      {:ok, opts} = Options.new(agent: ValidAgent, default_dispatch: {:logger, level: :info})

      assert opts.default_dispatch == {:logger, level: :info}
    end

    test "creates options with max_queue_size" do
      {:ok, opts} = Options.new(agent: ValidAgent, max_queue_size: 100)

      assert opts.max_queue_size == 100
    end

    test "creates options with on_parent_death" do
      {:ok, opts} = Options.new(agent: ValidAgent, on_parent_death: :continue)
    ...
    test "creates options with spawn_fun" do
      ...
      {:ok, opts} = Options.new(agent: ValidAgent, spawn_fun: spawn_fun)
  ...
  describe "new/1 with map" do
    test "creates options from map" do
      {:ok, opts} = Options.new(%{agent: ValidAgent, id: "map-test"})
  ...
  describe "new!/1" do
    test "returns options on success" do
      opts = Options.new!(agent: ValidAgent)
  ...
  describe "agent validation" do
    ...
    test "accepts agent_module for struct agents" do
      agent = ValidAgent.new()
      {:ok, opts} = Options.new(agent: agent, agent_module: ValidAgent)
  ...
  describe "ID handling" do
    ...
    test "uses provided ID" do
      {:ok, opts} = Options.new(agent: ValidAgent, id: "explicit-id")

      assert opts.id == "explicit-id"
    end

    test "converts atom ID to string" do
      {:ok, opts} = Options.new(agent: ValidAgent, id: :atom_id)
    ...
    test "prefers explicit ID over agent struct ID" do
      ...
      {:ok, opts} = Options.new(agent: agent, id: "explicit-id")

      assert opts.id == "explicit-id"
    end

    test "handles empty string ID by generating one" do
      {:ok, opts} = Options.new(agent: ValidAgent, id: "")
  ...
  describe "error policy validation" do
    test "accepts :log_only" do
      {:ok, opts} = Options.new(agent: ValidAgent, error_policy: :log_only)

      assert opts.error_policy == :log_only
    end

    test "accepts :stop_on_error" do
      {:ok, opts} = Options.new(agent: ValidAgent, error_policy: :stop_on_error)
    ...
    test "accepts {:emit_signal, config}" do
      ...
      {:ok, opts} = Options.new(agent: ValidAgent, error_policy: policy)

      assert opts.error_policy == policy
    end

    test "accepts {:max_errors, n}" do
      {:ok, opts} = Options.new(agent: ValidAgent, error_policy: {:max_errors, 5})
    ...
    test "rejects invalid error policy" do
      {:error, error} = Options.new(agent: ValidAgent, error_policy: :invalid)

      assert error.message =~ "invalid error_policy"
    end

    test "rejects {:max_errors, 0}" do
      {:error, error} = Options.new(agent: ValidAgent, error_policy: {:max_errors, 0})

      assert error.message =~ "invalid error_policy"
    end

    test "rejects {:max_errors, -1}" do
      {:error, error} = Options.new(agent: ValidAgent, error_policy: {:max_errors, -1})

      assert error.message =~ "invalid error_policy"
    end

    test "defaults to :log_only when nil" do
      {:ok, opts} = Options.new(agent: ValidAgent, error_policy: nil)
  ...
  describe "parent validation" do
    test "accepts nil parent" do
      {:ok, opts} = Options.new(agent: ValidAgent, parent: nil)
    ...
    test "accepts ParentRef struct" do
      ...
      {:ok, opts} = Options.new(agent: ValidAgent, parent: parent)
    ...
    test "accepts parent as map and converts to ParentRef" do
      {:ok, opts} =
        Options.new(
          agent: ValidAgent,
          parent: %{pid: self(), id: "parent-2", tag: :child}
        )
    ...
    test "rejects invalid parent" do
      {:error, error} = Options.new(agent: ValidAgent, parent: "invalid")
  ...
  describe "max_queue_size validation" do
    test "accepts positive integer" do
      {:ok, opts} = Options.new(agent: ValidAgent, max_queue_size: 500)
