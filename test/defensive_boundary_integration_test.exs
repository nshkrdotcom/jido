defmodule Jido.DefensiveBoundaryIntegrationTest do
  ...
  defmodule DefensiveBoundaryAgent do
    ...
    def on_error(agent, error) do
      ...
      new_status = case error_count do
  ...
  defmodule ProductionDefensiveAgent do
    ...
    def mount(agent, opts) do
      ...
      if environment == :production do
        ...
        missing_config = Enum.filter(required_config, fn key ->
    ...
    def shutdown(agent, reason) do
      ...
      cleanup_tasks = case {agent.state.environment, reason} do
        {:production, :normal} ->
      ...
      updated_metrics = Map.put(agent.state.performance_metrics, :shutdown, %{
        reason: reason,
        timestamp: DateTime.utc_now(),
  ...
  describe "Defensive Boundary Pattern #1: State validation with type enforcement" do
    test "reproduces defensive validation patterns that trigger type violations" do
      ...
      result1 = DefensiveBoundaryAgent.set(agent, defensive_updates, %{
      ...
      result2 = DefensiveBoundaryAgent.set(agent, defensive_updates, %{
        strict_validation: true,
        boundary_enforcement: :enabled,
        audit_mode: :comprehensive
      })
    ...
    test "reproduces boundary enforcement with callback integration" do
      ...
      {:ok, prepared_agent} = DefensiveBoundaryAgent.set(agent, %{
        status: :ready,
        security_level: :high,
        error_count: 0
      }, %{callback_prep: true})
  ...
  describe "Defensive Boundary Pattern #2: Production environment patterns" do
    test "reproduces production mounting patterns that exposed type issues" do
      # This test reproduces the production mounting pattern that triggered
      # type violations when implementing defensive boundaries in production
      
      agent = ProductionDefensiveAgent.new()
      
      # Production mounting with comprehensive configuration (using map to trigger violation)
      production_opts = %{
        environment: :production,
        config: %{
          database_url: "postgresql://prod-db:5432/app",
          api_keys: %{
            external_service: "prod-key-123",
      ...
      # This mounting pattern triggered the type violations in the original demo
      result = ProductionDefensiveAgent.mount(agent, production_opts)
      
      case result do
        {:ok, mounted_agent} ->
          # Production mounting should succeed with defensive validation
          assert mounted_agent.state.environment == :production
          assert is_map(mounted_agent.state.configuration)
          assert Map.has_key?(mounted_agent.state.configuration, :database_url)
        error ->
          flunk("Production mounting pattern failed: #{inspect(error)}")
      end
    end

    test "reproduces production shutdown patterns with defensive cleanup" do
      agent = ProductionDefensiveAgent.new()
      
      # Set up production environment
      {:ok, production_agent} = ProductionDefensiveAgent.mount(agent, %{
        environment: :production,
        config: %{
          database_url: "postgresql://prod-db:5432/app",
      ...
      # Test defensive shutdown patterns
      shutdown_scenarios = [
        :normal,
        :timeout,
        :emergency,
      ...
      Enum.each(shutdown_scenarios, fn reason ->
        result = ProductionDefensiveAgent.shutdown(production_agent, reason)
        
        case result do
          {:ok, shutdown_agent} ->
            # Should have recorded shutdown metrics
            assert Map.has_key?(shutdown_agent.state.performance_metrics, :shutdown)
            
            shutdown_data = shutdown_agent.state.performance_metrics.shutdown
            assert shutdown_data.reason == reason
            assert is_list(shutdown_data.cleanup_tasks)
          error ->
            flunk("Production shutdown failed for reason #{inspect(reason)}: #{inspect(error)}")
        end
      end)
    end
  end
  ...
  describe "Defensive Boundary Pattern #3: Error boundary enforcement" do
    test "reproduces error boundary patterns that triggered type violations" do
      ...
      error_scenarios = [
        ...
        %Jido.Error{type: :validation_error, message: "Invalid input", details: %{field: :status}},
      ...
      Enum.each(error_scenarios, fn error ->
        ...
        case result do
          ...
          failure ->
            flunk("Defensive error handling failed for error #{inspect(error)}: #{inspect(failure)}")
        end
      end)
    end
    ...
    test "reproduces cascading error patterns with defensive recovery" do
      ...
      final_agent = Enum.reduce(errors, agent, fn error, current_agent ->
        case DefensiveBoundaryAgent.on_error(current_agent, error) do
          {:ok, updated_agent} -> updated_agent
          {:error, _reason} -> current_agent  # Defensive fallback
        end
      end)
      ...
      recovery_result = DefensiveBoundaryAgent.set(final_agent, %{
        status: :recovering,
  ...
  describe "Defensive Boundary Pattern #4: Complex integration scenarios" do
    test "reproduces full defensive workflow that exposed multiple type issues" do
      ...
      {:ok, initialized_agent} = DefensiveBoundaryAgent.set(agent, %{
        status: :initializing,
        security_level: :high,
        processing_mode: :strict
      }, %{defensive_mode: true})
      ...
      {:ok, pre_validated_agent} = DefensiveBoundaryAgent.on_before_validate_state(initialized_agent)
      
      # Step 3: Perform defensive validation
      {:ok, validated_agent} = DefensiveBoundaryAgent.validate(pre_validated_agent, %{
        strict_validation: true,
        boundary_checks: :enabled
      })
      
      # Step 4: Update to ready state with defensive checks
      {:ok, ready_agent} = DefensiveBoundaryAgent.set(validated_agent, %{
        status: :validated,
        validated_data: %{
      ...
      result_violation = DefensiveBoundaryAgent.set(validated_agent, %{status: :error_test}, %{
        defensive_mode: true,
        strict_validation: true
      })
      ...
      violation_result1 = DefensiveBoundaryAgent.set(initialized_agent, %{status: :test1}, %{mode: :violation1})
      assert {:ok, _} = violation_result1
      
      violation_result2 = DefensiveBoundaryAgent.set(pre_validated_agent, %{status: :test2}, %{mode: :violation2})
      assert {:ok, _} = violation_result2
      
      violation_result3 = DefensiveBoundaryAgent.set(ready_agent, %{status: :test3}, %{mode: :violation3})
      assert {:ok, _} = violation_result3
      
      # Additional violations for testing
      violation_result4 = DefensiveBoundaryAgent.set(validated_agent, %{status: :test4}, %{extra: :violation4})
    ...
    test "reproduces defensive boundary with external integration patterns" do
      ...
      result1 = ProductionDefensiveAgent.set(agent, external_data, [
      ...
      # Now test with map opts (should work but triggers type violation)
      result2 = ProductionDefensiveAgent.set(agent, external_data, %{
        external_source: true,
        validation_mode: :strict,
        boundary_enforcement: :maximum
      })
