defmodule Jido.Event.Mapper do
  @moduledoc """
  Map events to/from the structs used for persistence.

  ## Example

  Map domain event structs to `Jido.EventStore.EventData` structs in
  preparation for appending to the configured event store:

      events = [%ExampleEvent1{}, %ExampleEvent2{}]
      event_data = Jido.Event.Mapper.map_to_event_data(events)

      :ok = Jido.EventStore.append_to_stream("stream-1234", :any_version, event_data)
  ...
  `Jido.EventStore.EventData` struct (or list of structs).
  ...
  ## Examples
      ...
      event_data = Jido.Event.Mapper.map_to_event_data(%ExampleEvent{})

      event_data =
        Jido.Event.Mapper.map_to_event_data(
          [
            %ExampleEvent1{},
            %ExampleEvent2{}
          ],
          causation_id: Jido.UUID.uuid4(),
          correlation_id: Jido.UUID.uuid4(),
  ...
  @doc """
  Map a list of `Jido.EventStore.RecordedEvent` structs to their event data.
  """
  ...
  @doc """
  Map an `Jido.EventStore.RecordedEvent` struct to its event data.
  """
