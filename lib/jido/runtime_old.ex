# defmodule Jido.Agent.RuntimeOld do
#   @moduledoc """
...
#   alias Jido.Agent.Runtime.Syscall
...
#       PubSub.emit(running_state, :started, %{agent_id: agent.id})
...
#     debug("Handling  signal", attrs: attrs, state: state)
...
#     debug("Handling synchronous cmd signal", attrs: attrs, from: from)

#     case process_cmd(attrs, state) do
#       {:ok, new_state} ->
#         debug("Command processed successfully", new_state: new_state)
#         IO.inspect(new_state.pending, label: "handle_call new_state.pending")
...
#           case queue_command(state, {:cmd, attrs}) do
...
#             PubSub.emit(idle_state, :cmd_completed, %{
#               command: Map.get(attrs, :command, :default),
#               result: new_agent
#             })

#             IO.inspect(new_agent, label: "process_cmd new_agent")
#             IO.inspect(idle_state, label: "process_cmd idle_state")
...
#             PubSub.emit(paused_state, :state_changed, %{from: :running, to: :paused})
#             {:ok, paused_state}
#           end
...
#             PubSub.emit(running_state, :state_changed, %{from: status, to: :running})
...
#         PubSub.emit(idle_state, :state_changed, %{from: state.status, to: :idle})
...
#     defp process_pending_commands(%{status: :idle, agent: %{pending: queue}} = state) do
#       debug("Processing pending commands", queue: queue)

#       case :queue.out(queue) do
#         {{:value, {:cmd, attrs}}, new_queue} ->
...
#     defp process_pending_commands(state) do
#       debug("Processing pending commands", state: state)
#       state
#     end
...
#       try do
#         {:ok, new_agent} = state.agent.__struct__.cmd(state.agent, command, params, opts)
#         # IO.inspect(new_agent, label: "new_agent")
#         syscall = new_agent.result.result
#         # IO.inspect(syscall, label: "syscall")
#         # debug("syscall", syscall: syscall)

#         {:ok, syscall_result} =
#           case syscall do
#             {:syscall, syscall} ->
#               case Syscall.execute(syscall, new_agent) do
#                 # Return just the agent part
#                 {:ok, new_state} ->
#                   # IO.inspect(new_state, label: "syscall_new_state")
#                   # IO.inspect(new_agent, label: "new_agent")
#                   # debug("Syscall new state", new_state: new_state)
#                   {:ok, new_state}

#                 error ->
#                   # debug("Syscall error", error: error)
#                   error
#               end

#             _ ->
#               debug("Non syscall result", result: new_agent.result.result)
#               {:ok, new_agent}
#           end

#         IO.inspect(new_agent.pending, label: "original_pending")
#         IO.inspect(syscall_result.pending, label: "syscall_result_pending")
#         # debug("result", result: syscall_result)
#         {:ok, syscall_result}
#         # {:ok, new_agent} = state.agent.__struct__.cmd(state.agent, command, params, opts)

#         # case new_agent.result.result do
#         #   {:syscall, syscall} ->
#         #     # debug("Executing syscall", syscall: syscall)
#         #     syscall_result = Syscall.execute(syscall, state)
#         #     # IO.inspect(syscall_result, label: "syscall_result")
#         #     # debug("Syscall result", result: syscall_result)
#         #     syscall_result
...
#       with {:ok, running_state} <- State.transition(state, :running) do
#         PubSub.emit(running_state, :state_changed, %{from: :idle, to: :running})
#         {:ok, running_state}
#       end
#     end
...
#         PubSub.emit(state, :queue_overflow, %{
#           queue_size: queue_size,
#           max_size: state.max_queue_size,
...
#     # end
#     def validate_state(%State{pubsub: nil}), do: {:error, "PubSub module is required"}
#     def validate_state(%State{agent: nil}), do: {:error, "Agent is required"}
